...
#pragma once
#include "imode.h"
template <auto& TLeds> //Ссылаемся на контейнер TLeds
class All : public IMode //Наследуем публичный класс IMode.
{
public:
  void toggle() const override // Override нужен поскольку переопределяем метод абстрактного класса IMode
  {
    TLeds.toggle();
  }
...
public:
  ...
  void init() const override
 {
    for ( uint32_t index =0; index < std::size(TLeds.pLeds); ++index)
  {
   if ((index %2)==0)
    {
     TLeds.pLeds[index]->toggle(); //Если получаем остаток от деления равный нулю, переключаем светодиоды.
    }
  }
 }
};
...
После прошивки платы можно наблюдать разные режимы мигания светодиодов при нажатие кнопки.
